...
class Api extends Homey.SimpleClass {
    ...
    constructor(homey) {
        ...
        this.intercoms = [];
        this.locations = [];
        this._tokenWatchdog = null;
        this._alarmModeIntervals = [];
        this._alarmModeFailureCounts = {};
    ...
    async _connectRingAPI() {
        ...
        try {
            ...
            // Watchdog: catch a refresh token going invalid even when no
            // camera/intercom onData event happens to fire
            if (!this._tokenWatchdog) {
                this._tokenWatchdog = this.homey.setInterval(() => this._checkRefreshToken(), 5000);
            }            
        }
        catch(error) {
    ...
    async _subscribeToAlarmMode(location) {
        ...
        const interval = this.homey.setInterval(() => this._refreshAlarmMode(location.locationDetails.location_id), (alarmModeInterval*1000));
        this._alarmModeIntervals.push(interval);
    ...
    async _getAlarmDevices(location) {
        ...
        try {
            ...
            this.devices = await Promise.race([
                location.getDevices(),
                new Promise((_, reject) =>
                    this.homey.setTimeout(() => reject(new Error(`getDevices() timed out for location: ${location.name}`)), timeoutMs)
                ),
            ]);
            ...
            const logLine = "Api.js || _getAlarmDevices || Error: " + error.message;
            this.homey.app.writeLog(logLine);
            // Don't rethrow: an unreachable/offline base station shouldn't block
            // init of other locations or leave the app hanging indefinitely.
        }
    }
    ...
    _disconnectRingAPI ( ) {
        ...
        if (this._tokenWatchdog) {
            this.homey.clearInterval(this._tokenWatchdog);
            this._tokenWatchdog = null;
        }
        this._alarmModeIntervals.forEach(interval => this.homey.clearInterval(interval));
        this._alarmModeIntervals = [];
        try {
    ...
    _checkRefreshToken () {
        if (this.ringApi && !this.ringApi.restClient.refreshToken) {
            this.log('_checkRefreshToken:        refreshToken lost');
            const report = JSON.parse('{"error": "refreshtoken not valid", "error_description": "The refreshtoken is invalid, please reauthenticate."}');
            this._setAuthenticationStatus(false, report);
        }
    }
    ...
    async _refreshAlarmMode (locationId) {
        ...
        const logEveryNthFailure = (6 * 60 * 60) / alarmModeInterval; // once every 6 hours
        ...
        try {
            ...
            for (const location of this.locations) {
                if (location.locationDetails.location_id == locationId) {
                    ...
                    const mode = await Promise.race([
                        location.getAlarmMode(), // no longer working :(
                        new Promise((_, reject) =>
                            this.homey.setTimeout(() => reject(new Error(`getAlarmMode() timed out for location: ${location.name}`)), timeoutMs)
                        ),
                    ]);
                    this._alarmModeFailureCounts[locationId] = 0;
            ...
            const failureCount = (this._alarmModeFailureCounts[locationId] || 0) + 1;
            this._alarmModeFailureCounts[locationId] = failureCount;

            if (failureCount === 1 || failureCount % logEveryNthFailure === 0) {
                this.log('_refreshAlarmMode:         Error:', error);
            }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (error.name === 'AbortError') {
                    this.log('_https_auth_cred: request timed out');
    ...
    _https_auth_code(auth, code) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                clearTimeout(timeout);
                if (error.name === 'AbortError') {
                    this.log('_https_auth_code: request timed out');
                    return reject(new Error('request_timeout'));
                }
                reject(error);
            }
    ...
    _setAuthenticationStatus (status, report) {
        ...
        if (this._previousAuthenticated != this._authenticated) {
            if (this._authenticated) {
                ...
                this.homey.settings.set('unauthenticatedNotified', false);
            } else {
                this.NoAuthStatLogTimeout = setTimeout(async () => {
                    ...
                    if (!this.homey.settings.get('unauthenticatedNotified')) {
                        await this.homey.notifications.createNotification({
                            excerpt: this.homey.__('notifications.unauthenticated'),
                        });
                        this.homey.settings.set('unauthenticatedNotified', true);
                    }
    ...
    //console.log('location::::',location.locationDetails.name)                
            try {
                ...
                const result = await Promise.race([
                    location.getDevices(),
                    new Promise((_, reject) =>
                        timer = this.homey.setTimeout(() => reject(new Error(`getDevices() timed out for location: ${location.name}`)), timeoutMs)
                        //this.homey.setTimeout(() => reject(this.log('timout reached')), timeoutMs)
                    ),
                ]);
    ...
    //console.log('devices::::',result)                
                ...
                this.log('getAlarmDevices:           Failed for location:', location.name, ':', error.message);
            }
        }
    ...
    async unlock(device_data) {
        try {
            ...
            if (!result?.result) {
                this.log('unlock: unexpected response', result);
                return false;
            }
    ...
    async grabImage(device_data) {
        ...
        try {
            ...
            if (this._streamingCameraIds?.has(device_data.id)) {
                throw new Error(`Api.js grabImage: Skipped snapshot for ${device_data.id} — video stream is active`);
            }
    ...
    async grabVideo(device_data, offerSdp) {
        ...
        this._streamingCameraIds ??= new Set();
        this._streamingCameraIds.add(device_data.id);        
        try {
            const camera = this.cameras.find(c => c.initialData.id === device_data.id);
...
// this.log(camera.data);
            ...
            const isMainsPowered = this._isCameraMainsPowered(camera);
            this.log('Api.js grabVideo Camera is mains powered:', isMainsPowered);

            if (!isMainsPowered) {
                throw new Error('Live video not supported on battery operated cameras');
            }
            ...
            this._streamingCameraIds.delete(device_data.id);
        }
    }  
    ...
    _isCameraMainsPowered(camera) {
        ...
        if (data.settings.power_mode === 'wired') return true;
        ...
        const batteryKnown = health.battery_percentage_category !== 'unknown' && health.battery_voltage_category !== 'unknown';
        ...
        if (kind.includes('wired') || kind.includes('plug_in') || kind.includes('elite') || kind.startsWith('hp_') || kind.includes('mini') || kind.startsWith('spotlightw')) return true;
        ...
        for (const camera of this.cameras) {
            if (camera.initialData.id === device_data.id) {
                try {
                    ...
                    if (error.response) {
                        this.error('Ring API response status:', error.response.status);
                        this.error('Ring API response body:', JSON.stringify(error.response.body));
                    }
                    throw error;
                }
            }
    ...
    async useFloodlightOnMotion(device_data,useFloodlightOnMotion) {
        //this.log('API useFloodlightOnMotion',useFloodlightOnMotion)
        for (const camera of this.cameras) {
            if (camera.initialData.id === device_data.id) {
                // The setting is inverted, it acts as snooze on/off
                const newSetting = { light_snooze_settings: { always_on: !useFloodlightOnMotion } };
                return await camera.setDeviceSettings(newSetting);
            }
        }
        throw new Error(`Camera with id ${device_data.id} not found`);
    }
